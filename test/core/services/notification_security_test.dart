    ...
    group('User ID Validation Security', () {
      ...
      test('should accept valid Firebase Auth UIDs', () async {
        ...
        when(mockMessaging.getNotificationSettings()).thenAnswer(
          (_) async => const fm.NotificationSettings(
            authorizationStatus: fm.AuthorizationStatus.authorized,
            alert: fm.AppleNotificationSetting.enabled,
            badge: fm.AppleNotificationSetting.enabled,
            sound: fm.AppleNotificationSetting.enabled,
            announcement: fm.AppleNotificationSetting.disabled,
            carPlay: fm.AppleNotificationSetting.disabled,
            lockScreen: fm.AppleNotificationSetting.enabled,
            notificationCenter: fm.AppleNotificationSetting.enabled,
            showPreviews: fm.AppleShowPreviewSetting.always,
            criticalAlert: fm.AppleNotificationSetting.disabled,
            timeSensitive: fm.AppleNotificationSetting.disabled,
            providesAppNotificationSettings: fm.AppleNotificationSetting.enabled,
          ),
        );
    ...
    group('Permission Status Security', () {
      test('should properly validate notification permissions', () async {
        ...
        for (final status in permissionTests) {
          when(mockMessaging.getNotificationSettings()).thenAnswer(
            (_) async => fm.NotificationSettings(
          ...
          switch (status) {
            case fm.AuthorizationStatus.authorized:
            case fm.AuthorizationStatus.provisional:
              expect(result, equals(NotificationPermissionStatus.granted));
              break;
            case fm.AuthorizationStatus.denied:
              expect(result, equals(NotificationPermissionStatus.denied));
              break;
            case fm.AuthorizationStatus.notDetermined:
              expect(
                result,
    ...
    group('Topic Subscription Security', () {
      test('should validate topic names before subscription', () async {
        ...
        when(mockMessaging.subscribeToTopic(_any<String>()))
            .thenAnswer((_) async {});
        when(mockMessaging.unsubscribeFromTopic(_any<String>()))
            .thenAnswer((_) async {});
  ...
  group('VIP Notification Performance & Security Tests', () {
    ...
    group('Performance Optimization Tests', () {
      test('should use pagination for large user sets', () async {
        ...
        when(mockQuery.limit(_any<int>())).thenReturn(mockQuery);
        when(mockQuery.get()).thenAnswer((_) async => mockSnapshot1);
        when(mockQuery.get()).thenAnswer((_) async => mockSnapshot2);
    ...
    group('High-Value Sweepstake Security', () {
      test('should validate high-value threshold', () async {
        ...
        when(mockCollection.doc(_any<String>())).thenReturn(mockDocRef);
        when(mockDocRef.set(_any<Map<String, dynamic>>()))
            .thenAnswer((_) async {});
        ...
        verify(mockDocRef.set(_any<Map<String, dynamic>>()))
