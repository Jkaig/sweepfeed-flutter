...
class DustBunniesService extends ChangeNotifier {
    ...
    try {
      ...
      try {
        await _firestore.runTransaction((transaction) async {
          ...
          int currentDB = (currentData['currentDB'] as int?) ?? 0;
          int totalDB = (currentData['totalDB'] as int?) ?? 0;
          int currentLevel = (currentData['level'] as int?) ?? 1;
          double multiplier = (currentData['multiplier'] as double?) ?? 1.0;
          ...
          int dbGained = (baseDB * finalMultiplier).round();
  ...
  Future<Map<String, dynamic>> getUserSweepPointsData(String userId) async {
    return getUserDustBunniesData(userId);
  }
  ...
  Future<void> applySweepPointsBooster(
    ...
    return applyDustBunniesBooster(userId, multiplier, duration);
  }
    ...
    try {
      ...
      if (totalEntries >= entryThreshold) {
        ...
        if (!achievements.containsKey('entry_enthusiast')) {
          ...
          analyticsCallback?.call('achievement_earned', {
  ...
  /// Award DustBunnies specifically for sweep/contest-related activities
  ///
  /// This is a specialized method for awarding DustBunnies for activities
  /// related to sweepstakes and contests (e.g., entering contests, sharing).
  ///
  /// @param userId The ID of the user to award points to
  /// @param amount The amount of DustBunnies to award
  /// @param reason The reason for awarding these points (for tracking)
  /// @returns DustBunniesReward with details of points awarded
  Future<DustBunniesReward> awardSweepDust({
    required String userId,
    required int amount,
    required String reason,
  }) async {
    try {
      // Award DustBunnies using the existing awardDustBunnies method
      final reward = await awardDustBunnies(
        userId: userId,
        action: reason,
        customAmount: amount,
      ...
      analyticsCallback?.call('sweep_dust_awarded', {
        'user_id': userId,
        'amount': amount,
        'reason': reason,
      });
      ...
      logger.e('Error awarding sweep dust to $userId', error: e);
      return const DustBunniesReward(
        pointsAwarded: 0,
        leveledUp: false,
      );
