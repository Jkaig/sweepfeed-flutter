...
class EntryManagementService {
  factory EntryManagementService() => _instance;
  EntryManagementService._internal();
  static final EntryManagementService _instance =
      EntryManagementService._internal();
  ...
  Future<ContestEntry> submitEntry({
    ...
    final entry = ContestEntry(
      ...
      isDailyEntry: sweepstake.isDailyEntry,
      nextEntryAllowed:
          sweepstake.isDailyEntry ? now.add(const Duration(days: 1)) : null,
      createdAt: now,
      updatedAt: now,
    );
    ...
    await _trackEntryAnalytics(savedEntry);
  ...
  Future<List<ContestEntry>> submitBulkEntries({
    required List<Sweepstakes> sweepstakes,
    required EntryMethod method,
    ...
    batch.set(bulkEntryDoc, {
      'userId': user.uid,
      'contestCount': sweepstakes.length,
      'method': method.toString().split('.').last,
      'status': 'processing',
      'createdAt': FieldValue.serverTimestamp(),
    });

    for (final sweepstake in sweepstakes) {
