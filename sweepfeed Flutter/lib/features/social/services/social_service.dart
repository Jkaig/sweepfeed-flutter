...
class SocialService {
  ...
  Stream<List<SweepstakesComment>> getComments(String sweepstakesId) =>
      _firestore
          .collection('sweepstakes')
          .doc(sweepstakesId)
          .collection('comments')
          ...
          .map(
            (snapshot) => snapshot.docs
                .map((doc) => SweepstakesComment.fromMap(doc.data(), doc.id))
                .toList(),
          );

  Stream<List<SweepstakesComment>> getHelpfulComments(String sweepstakesId) =>
      _firestore
          .collection('sweepstakes')
          .doc(sweepstakesId)
          .collection('comments')
  ...
  Future<String> postReferralCode({
    ...
    // Check if user already posted a code for this sweepstakes
    ...
    if (existingCode.docs.isNotEmpty) {
      throw Exception(
        'You already posted a referral code for this sweepstakes',
      );
    }
  ...
  Future<void> useReferralCode({
    ...
    // Check if user already used a code for this sweepstakes
    final userDoc = await _firestore.collection('users').doc(user.uid).get();
    final userData = userDoc.data();
  ...
  Stream<List<ReferralCode>> getReferralCodes(String sweepstakesId) =>
      _firestore
          .collection('sweepstakes')
          .doc(sweepstakesId)
          .collection('referralCodes')
