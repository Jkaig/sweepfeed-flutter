...
import '../../../features/contests/models/filter_options.dart';
...
class ContestRepository {
  ...
  Future<List<Contest>> getFilteredContests({
    ...
    try {
      ...
      // --- Custom Filters ---
      if (filterOptions.requiresPurchase != null) {
        query = query.where('requiresPurchase',
            isEqualTo: filterOptions.requiresPurchase,);
      }
      query = query.where('prizeValueUsd',
          isGreaterThanOrEqualTo: filterOptions.minPrizeValue,);
          query = query.where('prizeValueUsd',
          isLessThanOrEqualTo: filterOptions.maxPrizeValue,);
          if (filterOptions.entryMethods.isNotEmpty) {
        query = query.where('entryMethods',
            arrayContainsAny: filterOptions.entryMethods.toList(),);
      }
      if (filterOptions.platforms.isNotEmpty) {
        query = query.where('platform', whereIn: filterOptions.platforms.toList());
      }
      ...
      switch (filterOptions.sortOption) {
        ...
        case SortOption.prizeValueHighToLow:
          query = query.orderBy('prizeValueUsd', descending: true);
          break;
        case SortOption.prizeValueLowToHigh:
          query = query.orderBy('prizeValueUsd', descending: false);
          break;
      ...
      final snapshot = await _executeQueryWithTimeout(query, 'getFilteredContests');
      return _parseContestSnapshot(snapshot);
    } on FirebaseException catch (e) {
      logger.e('Firebase error getting filtered contests: ${e.code}', error: e);
      return [];
    } on Exception catch (e) {
      logger.e('Error getting filtered contests', error: e);
      return [];
    }
  }
