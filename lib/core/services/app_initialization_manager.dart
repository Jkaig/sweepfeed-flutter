  ...
  Future<void> initializeBackground() async {
    ...
    try {
      ...
      try {
        ...
        logger.d('AdMob SDK initialized');
      } catch (e) {
        logger.w(
          'AdMob initialization failed (non-critical)',
          error: e,
        );
      }

      // RevenueCat initialization is handled by AuthService after login
      // and by a provider that watches auth state changes
      // No need to initialize here as it requires a user ID
      logger.d('RevenueCat will be initialized on user login');
