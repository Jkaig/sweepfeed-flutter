...
class StreakService extends ChangeNotifier {
  StreakService(this._dustBunniesService);
  ...
  // Add freezes (e.g. from shop purchase)
  Future<void> addFreeze(String userId, {int amount = 1}) async {
    try {
      await _firestore.collection('users').doc(userId).update({
        'streaks.freezesAvailable': FieldValue.increment(amount),
      });
      notifyListeners();
    } catch (e) {
      logger.e('Error adding freeze', error: e);
    }
  }
