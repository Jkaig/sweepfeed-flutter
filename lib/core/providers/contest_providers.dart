...
final enteredContestIdsProvider = StreamProvider<Set<String>>((ref) {
  final auth = ref.watch(authServiceProvider);
  final user = auth.currentUser;
  
  if (user == null) return Stream.value({});
  
  final entryService = ref.watch(entryManagementServiceProvider);
  return entryService.getEnteredContestIdsStream(user.uid);
});
...
final activeContestsStreamProvider =
  ...
  final enteredIdsAsync = ref.watch(enteredContestIdsProvider);
  ...
  return repository.getActiveContestsStream(
    ...
    // Filter out entered contests
    final enteredIds = enteredIdsAsync.value ?? {};
    if (enteredIds.isEmpty) return contests;
    
    return contests.where((c) => !enteredIds.contains(c.id)).toList();
  });
});
