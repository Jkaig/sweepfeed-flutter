...
class _EmailInboxScreenState extends ConsumerState<EmailInboxScreen>
  ...
  final _searchResultsProvider =
      StateProvider<List<EmailMessage>?>((ref) => null);
  ...
  Widget _buildSelectionToolbar() => Container(
  ...
  Widget _buildEmailList(EmailCategory? category) {
    ...
    if (_isSearching && searchResults == null) {
      return const Center(child: LoadingIndicator());
    }

    final emails = (_isSearching ? searchResults : emailsAsync.asData?.value) ?? [];
    ...
    if (_isSearching) {
      ...
      switch (category) {
        case EmailCategory.promo:
          ...
          subtitle =
              'Promotional emails will appear here when you receive them.';
          icon = Icons.local_offer;
          break;
        ...
        default:
          ...
          subtitle =
              'Start using your @sweepfeed.app email address to receive emails here.';
          icon = Icons.inbox;
          break;
      }
    }
  ...
  Widget _buildUpgradeScreen() => Scaffold(
      backgroundColor: AppColors.primaryDark,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                "We will let you know if you win and also keep track of any juicy coupons you get, don't worry! The email inbox is exclusively available to Premium subscribers. Get your own @sweepfeed.app email address and never miss important contests notifications!",
  ...
  void _performSearch(String query) {
    final allEmails = ref.read(emailsStreamProvider(null)).asData?.value ?? [];
    final filteredEmails = allEmails.where((email) {
      final queryLower = query.toLowerCase();
      return email.subject.toLowerCase().contains(queryLower) ||
          email.from.toLowerCase().contains(queryLower);
    }).toList();
    ref.read(_searchResultsProvider.notifier).state = filteredEmails;
  }
