...
class ShopItemCard extends StatefulWidget {
  const ShopItemCard({
    required this.item,
    required this.onViewDetails, this.onPurchase,
    super.key,
  });
  final ShopItem item;
...
class _ShopItemCardState extends State<ShopItemCard>
  ...
  Color _getBorderColor() {
    ...
    } else if (widget.item.type == ShopItemType.powerUp || 
               widget.item.type == ShopItemType.utility) {
       return const Color(0xFF00E5FF); // Brand Cyan for tools
    } else {
  ...
  LinearGradient _getCardGradient() {
    if (widget.item.isOwned) {
      ...
      return LinearGradient(
        ...
        stops: const [0.02, 0.02], // Subtle top border effect
      );
    } else if (widget.item.isLimited) {
      return const LinearGradient(
        begin: Alignment.topLeft,
