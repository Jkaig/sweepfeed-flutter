...
/// Provider to get user's interests from their profile for feed personalization
final userInterestsProvider = Provider<List<String>>((ref) {
  final userProfileAsync = ref.watch(userProfileProvider);
  return userProfileAsync.whenOrNull(
        data: (profile) => profile?.interests ?? [],
      ) ??
      [];
});
...
final contestFeedProvider = FutureProvider<List<Contest>>((ref) async {
  ...
  // Filter out entered contests (remove from feed after entry)
  final user = ref.watch(firebaseAuthProvider).currentUser;
  if (user != null) {
    final entryService = ref.watch(entryServiceProvider);
    // Get all entered contest IDs in one efficient query
    final enteredContestIds = await entryService.getEnteredContestIds(user.uid);
    
    // Remove entered contests from feed
    filtered = filtered.where((c) => !enteredContestIds.contains(c.id)).toList();
  }

  // Apply interest-based personalization (boost matching contests to top)
  final isPersonalized = ref.watch(personalizedFeedEnabledProvider);
  final userInterests = ref.watch(userInterestsProvider);

  if (isPersonalized && userInterests.isNotEmpty) {
    // Create a lowercase set for efficient matching
    final interestsLower = userInterests.map((i) => i.toLowerCase()).toSet();

    // Helper to check if contest matches user interests
    bool matchesInterests(Contest contest) {
      // Check primary category
      if (interestsLower.contains(contest.category.toLowerCase())) {
        return true;
      }
      // Check categories list
      for (final cat in contest.categories) {
        if (interestsLower.contains(cat.toLowerCase())) {
          return true;
        }
      }
      return false;
    ...
    // Separate into matching and non-matching, preserving existing order
    final matching = <Contest>[];
    final nonMatching = <Contest>[];
    for (final contest in filtered) {
      if (matchesInterests(contest)) {
        matching.add(contest);
      } else {
        nonMatching.add(contest);
      }
    }

    // Combine with matching contests first
