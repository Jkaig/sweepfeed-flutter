...
class _ForYouFeedState extends ConsumerState<ForYouFeed>
  ...
  List<Contest> _personalizedContests = [];
  ...
  Future<List<Contest>> _generatePersonalizedContests() async {
  ...
  List<Contest> _generateMockContests() {
    final random = Random();
    ...
    for (var i = 0; i < contestTemplates.length; i++) {
      ...
      final contest = Contest.fromMap(template, 'contest_$i');

      // Add dynamic fields
      final updatedContest = contest.copyWith(
        entryCount: 1000 + random.nextInt(9000),
  ...
  Widget _buildContestsList() => ListView.builder(
        ...
        itemCount: _getFilteredContests().length,
        itemBuilder: (context, index) {
          final contest = _getFilteredContests()[index];
          return UnifiedContestCard(
            contest: contest,
            style: CardStyle.addictive,
            index: index,
