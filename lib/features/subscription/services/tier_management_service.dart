...
class TierManagementService with ChangeNotifier {
  ...
  bool hasLeaderboardAccess() => getCurrentTier().hasLeaderboardAccess;

  /// Check if user has access to social challenges
  bool hasSocialChallengesAccess() => getCurrentTier().hasSocialChallengesAccess;

  /// Check if user has ad-free experience
  bool hasAdFreeExperience() => getCurrentTier().isAdFree;

  /// Check if user has auto-entry scheduling
  bool hasAutoEntryScheduling() => getCurrentTier().hasAutoEntryScheduling;

  /// Check if user has exclusive partner contests access
  bool hasExclusivePartnerSweepstakes() => getCurrentTier().hasExclusivePartnerSweepstakes;

  /// Check if user has analytics features
  bool hasAnalytics() => getCurrentTier().hasAnalytics;

  /// Check if user has priority customer support
  bool hasPrioritySupport() => getCurrentTier().hasPrioritySupport;

  /// Get the SweepCoins multiplier for the current tier
  double getSweepCoinsMultiplier() => getCurrentTier().sweepCoinsMultiplier;
